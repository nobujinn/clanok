...
\title{Engagement in digitalnnn marketing: current advertising trends and recommendation algorithms} % meno a priezvisko vyučujúceho na cvičeniach
...
\begin{figure}[h]  % Начинаем окружение для вставки рисунка
    \centering     % Центрируем изображение
    \includegraphics[width=0.5\textwidth]{STU-FIIT-anch.png}  % Вставляем изображение, указывая ширину в процентах от ширины текста
    \caption{descr}  % Добавляем подпись к рисунку
    \label{fig:example}  % Можем ссылаться на изображение с помощью \ref{fig:example}
\end{figure}
...
Základným problémom je\ldots{} Najprv sa pozrieme na nejaké vysvetlenie (časť~\ref{ina:nejake}), a potom na ešte nejaké (časť~\ref{ina:nejake}).\footnote{Niekedy môžete potrebovať aj poznámku pod čiarou.}
